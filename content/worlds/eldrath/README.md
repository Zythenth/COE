...
Esta pasta reúne o pacote autoral que definirá as condições iniciais de Eldrath. A Fase 1 foi concluída após auditoria e consolidação do manifesto, dos contratos e dos templates. A Fase 2 está em andamento: [`world.yaml`](world.yaml), [`calendars/marco.md`](calendars/marco.md), os três idiomas fundamentais e as quatro culturas fundamentais estão `approved`; [`kingdom.yaml`](kingdom.yaml) está `in_review`. Geografia ainda não foi iniciada.
...
`world.eldrath` preserva a identidade geral aprovada — mundo vivo, causalidade, conhecimento limitado, magia sistêmica, instituições relevantes, vida cotidiana, conflito consequente, transformação histórica, variedade de trajetórias e ausência de protagonista obrigatório — e registra, em `notes` estruturadas, a cosmologia mínima e os princípios religiosos gerais aprovados. Essa camada distingue verdade confirmada, fenômeno observável, interpretação, crença, doutrina futura e mistério aberto; não cria uma religião nem confirma divindades ou pós-vida. O campo de calendário principal referencia `calendar.marco`; `language_ids` referencia `language.valdren`, `language.ardren` e `language.senkar`; `culture_ids` referencia `culture.dorven`, `culture.namaru`, `culture.tessari` e `culture.ostelar`; e `initial_kingdom_id` referencia `kingdom.averen`, conforme a propriedade canônica do contrato de mundo. Data inicial e era inicial permanecem nulas. Nenhuma unidade concreta foi definida nesta atividade.

`kingdom.averen`, a Coroa Pactuada de Averen, define o único reino inicial como monarquia pactuada e limitada. A Coroa coordena a execução; o Conselho do Pacto delibera e confirma matérias protegidas; o Tribunal de Garantias revisa competência, procedimento e sucessão; e o Registro da Coroa preserva atos e provas sem ser infalível. Os órgãos permanentes que precisarem de identidade própria serão materializados futuramente como `faction.*`, sem IDs antecipados. A sucessão combina primogenitura absoluta, reconhecimento jurídico, confirmação institucional, regência, interregno, disputa e restauração sem escolher família, governante ou sucessor concreto. Capital, regiões, moeda, tesouro, métricas, data inicial, leis e demais estados atuais continuam nulos ou vazios.
...
| `calendars/` | Calendário, eras e representação de datas. | 1 entidade real, `calendar.marco`, aprovada |
| `languages/` | Idiomas relevantes do mundo. | 3 entidades reais aprovadas: `language.valdren`, `language.ardren` e `language.senkar`; `.gitkeep` removido |
| `cultures/` | Culturas, normas, valores e contextos sociais. | 4 entidades reais aprovadas: `culture.dorven`, `culture.namaru`, `culture.tessari` e `culture.ostelar`; `.gitkeep` removido |
...
| `religions/` | Tradições religiosas, cultos como crença, doutrinas, práticas e diversidade. | schema e template disponíveis; contém somente `.gitkeep`; zero religiões; entidades reais não iniciadas |
