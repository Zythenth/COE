...
Esta pasta reúne o pacote autoral que definirá as condições iniciais de Eldrath. A Fase 1 foi concluída após auditoria e consolidação do manifesto, dos contratos e dos templates. A Fase 2 está em andamento: [`calendars/marco.md`](calendars/marco.md), os três idiomas fundamentais e as quatro culturas fundamentais estão `approved`; [`world.yaml`](world.yaml) está `in_review` somente pela nova camada cosmológica mínima e pelos princípios religiosos gerais. Reino e geografia ainda não foram iniciados.
...
`world.eldrath` preserva a identidade geral aprovada — mundo vivo, causalidade, conhecimento limitado, magia sistêmica, instituições relevantes, vida cotidiana, conflito consequente, transformação histórica, variedade de trajetórias e ausência de protagonista obrigatório — e agora registra, em `notes` estruturadas, a cosmologia mínima e os princípios religiosos gerais. Essa camada distingue verdade confirmada, fenômeno observável, interpretação, crença, doutrina futura e mistério aberto; não cria uma religião nem confirma divindades ou pós-vida. O campo de calendário principal referencia `calendar.marco`; `language_ids` referencia `language.valdren`, `language.ardren` e `language.senkar`; e `culture_ids` referencia `culture.dorven`, `culture.namaru`, `culture.tessari` e `culture.ostelar`, conforme a propriedade canônica do contrato de mundo. Data inicial, era inicial e reino permanecem sem referências. Nenhuma unidade concreta foi definida nesta atividade.
...
A Fase 0 e a **Fase 1 — Contratos editoriais e templates** estão concluídas. A auditoria editorial final aprovou e consolidou o contrato comum, o manifesto, os schemas especializados, os templates e os contratos conceituais transversais. A **Fase 2 — Fundamentos do mundo** está em andamento. `calendar.marco`, `language.valdren`, `language.ardren`, `language.senkar`, `culture.dorven`, `culture.namaru`, `culture.tessari` e `culture.ostelar` estão `approved`. `world.eldrath` está `in_review`: sua identidade geral anteriormente aprovada foi preservada, e somente a nova camada cosmológica e religiosa geral aguarda aprovação. São nove entidades reais no pacote. O reino definirá moedas internas; nenhuma unidade concreta foi criada nesta atividade. `kingdom.yaml` continua inexistente, e regiões, assentamentos, facções, religiões e NPCs não foram criados. Templates definem estruturas de autoria e não são conteúdo aprovado; o ciclo editorial é `draft → in_review → approved → deprecated`, e `approved` exige revisão explícita, referências resolvidas e ausência de placeholders. O contrato GitHub não altera o pacote nem suas contagens. A próxima atividade é aprovar a cosmologia e os princípios religiosos gerais, restaurar `world.eldrath` para `approved` e criar o reino, seu sistema político e suas regras de sucessão.
