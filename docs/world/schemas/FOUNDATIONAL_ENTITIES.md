...
**Campos obrigatórios.** Além dos campos comuns obrigatórios: `identity` (mapa), `primary_calendar_id` (ID `calendar.*` ou `null` em rascunho), `initial_date` (string de data do mundo ou `null`), `initial_era_id` (ID interno de era ou `null`), `language_ids` (lista de IDs `language.*`), `culture_ids` (lista de IDs `culture.*`), `initial_kingdom_id` (ID `kingdom.*` ou `null`), `units` (mapa) e `references` (lista).

`identity` possui `world_name` (string não vazia) e pode incluir `native_name` (string). `units` declara unidades aplicáveis por domínio como strings ou `null` enquanto não definidas; não substitui valores numéricos. `tags` continua uma lista, inclusive quando vazia.
...
**Referências permitidas.** Regiões usam `region.*`; idiomas usam `language.*`; relações religiosas, profissionais, festivais e ritos usam IDs completos quando as respectivas entidades existirem e seus schemas forem definidos. Não se registram nomes como referências.
...
**Campos obrigatórios.** Além dos campos comuns obrigatórios: `demonym` (string ou `null`), `symbol` (mapa ou `null`), `colors` (lista de strings), `motto` (string ou `null`), `foundation` (mapa ou `null`), `current_era_id` (ID interno de era ou `null`), `political_system` (mapa ou `null`), `succession` (mapa ou `null`), `capital_id` (ID `settlement.*` ou `null`), `ruler_id` (ID `npc.*` ou `null`), `council_ids` (lista de IDs), `general_law_ids` (lista de IDs `law.*`), `magical_law_ids` (lista de IDs `law.*`), `dominant_religion_id` (ID `religion.*` ou `null`), `tolerated_religion_ids` (lista de IDs `religion.*`), `prohibited_religion_ids` (lista de IDs `religion.*`), `currency_id` (ID futuro ou `null`), `calendar_id` (ID `calendar.*` ou `null`), `language_ids` (lista de IDs `language.*`), `region_ids` (lista de IDs `region.*`), `external_relations` (lista de mapas), `metrics` (mapa), `treasury` (mapa ou `null`), `administrative_capacity` (número na escala aplicável ou `null`), `initial_crisis_ids` (lista de IDs) e `founding_event_ids` (lista de IDs `event.*`).

`metrics` contém `stability`, `legitimacy`, `prosperity`, `security`, `cohesion`, `freedom`, `inequality`, `magical_tension`, `trust_in_crown` e `religious_influence`, como números de `0` a `100` ou `null`. `foundation`, `political_system`, `succession`, `symbol`, `treasury` e cada relação externa usam mapas estruturados, com referências por ID quando apontarem a outra entidade.
...
**Referências permitidas.** Capital, governante, conselho, leis, religiões, calendário, idiomas, regiões, crises e eventos fundadores usam IDs completos. `current_era_id` é interno ao calendário. A moeda só poderá apontar para o prefixo definido pelo schema econômico futuro; até então é `null`. O reino não incorpora documentos completos de outras entidades.

**Invariantes e validação manual.** Métricas usam a escala `0`–`100` definida no GDD; métrica desconhecida é `null`, nunca `0`. Capital, governante, religiões, regiões e leis são referências, não dados duplicados. O estado inicial é conteúdo autoral; mutações posteriores pertencem ao estado da simulação. Sucessores políticos são entidades ou relações posteriores e não sobrescrevem a história do reino original. A revisão confere intervalo, ausência correta, prefixos, referências resolvidas no aprovado e ausência de saves ou estado derivado.

**Decisões adiadas.** Gentílico, símbolo, cores, lema, fundação, capital, governante, religião, moeda, leis, métricas e crises reais não são definidos neste schema.
