...
**Campos obrigatórios.** Além dos campos comuns obrigatórios: `demonym` (string ou `null`), `symbol` (mapa ou `null`), `colors` (lista de strings), `motto` (string ou `null`), `foundation` (mapa ou `null`), `current_era_id` (ID interno de era ou `null`), `political_system` (mapa ou `null`), `succession` (mapa ou `null`), `capital_id` (ID `settlement.*` ou `null`), `ruler_id` (ID `npc.*` ou `null`), `council_ids` (lista de IDs), `general_law_ids` (lista de IDs `law.*`), `magical_law_ids` (lista de IDs `law.*`), `dominant_religion_id` (ID `religion.*` ou `null`), `tolerated_religion_ids` (lista de IDs `religion.*`), `prohibited_religion_ids` (lista de IDs `religion.*`), `primary_currency_key` (chave interna ou `null`), `currencies` (lista de mapas), `calendar_id` (ID `calendar.*` ou `null`), `language_ids` (lista de IDs `language.*`), `region_ids` (lista de IDs `region.*`), `external_relations` (lista de mapas), `metrics` (mapa), `treasury` (mapa monetário inicial), `administrative_capacity` (número na escala aplicável ou `null`), `initial_crisis_ids` (lista de IDs) e `founding_event_ids` (lista de IDs `event.*`).
...
`treasury` registra somente um valor inicial no formato `money: { amount, currency_key }`. Saldo atual, câmbio atual, inflação e dívida pertencem ao estado futuro. Os contratos monetários completos estão em [MATERIAL_ECONOMY_ENTITIES.md](MATERIAL_ECONOMY_ENTITIES.md).
...
**Referências permitidas.** Capital, governante, conselho, leis, religiões, calendário, idiomas, regiões, crises e eventos fundadores usam IDs completos. `current_era_id` é interno ao calendário. Moedas usam chaves internas estáveis do próprio reino, não IDs de entidade. O reino não incorpora documentos completos de outras entidades.

**Invariantes e validação manual.** Métricas usam a escala `0`–`100` definida no GDD; métrica desconhecida é `null`, nunca `0`. Chaves de moedas e subdivisões são únicas; a moeda principal existe na lista; fatores conhecidos são positivos; `currency_key` monetária resolve no reino aplicável. Capital, governante, religiões, regiões e leis são referências, não dados duplicados. O estado inicial é conteúdo autoral; mutações posteriores pertencem ao estado da simulação. Sucessores políticos são entidades ou relações posteriores e não sobrescrevem a história do reino original. A revisão confere intervalos, ausência correta, chaves, referências resolvidas no aprovado e ausência de saves ou estado derivado.

**Decisões adiadas.** Gentílico, símbolo, cores, lema, fundação, capital, governante, religião, moedas, subdivisões, relações monetárias, leis, métricas e crises reais não são definidos neste schema.
