...
Referências de entidade usam IDs completos, como `kingdom_id`, `region_id`, `settlement_id`, `parent_location_id`, `resource_id`, `origin_id` e `destination_id`. Chaves controladas usam valores de vocabulário futuro, nunca IDs inventados: `biome_key`, `climate_key`, `terrain_keys`, `settlement_type`, `location_type`, `route_type` e `access_modes`. Onde este schema mencionar uma chave controlada, o vocabulário de valores permanece decisão adiada, salvo as categorias mínimas expressamente listadas.

Presença, influência, atuação e controle institucionais iniciais pertencem a `faction.initial_state.territorial_presence`, conforme [SOCIETY_INSTITUTIONS_AND_LAW_ENTITIES.md](SOCIETY_INSTITUTIONS_AND_LAW_ENTITIES.md). Leis aplicáveis pertencem a `law.jurisdiction`; difusão religiosa inicial pertence a `religion.initial_state.diffusion`. As entidades geográficas não mantêm essas direções inversas.
...
**Campos obrigatórios.** Além dos campos comuns aplicáveis: `kingdom_id` (ID `kingdom.*`), `biome_key` (chave controlada), `climate_key` (chave controlada), `terrain_keys` (lista de chaves controladas), `initial_state` (mapa), `magical_traditions` (lista de chaves controladas), `resource_presence` (lista de mapas), `borders` (lista de mapas), `reputation` (número ou `null`, com escala adiada) e `references` (lista).
...
Cada entrada de `resource_presence` possui `resource_id` (`resource.*`), `abundance`, `accessibility` e `seasonality` (chaves controladas) e `notes` opcional. Uma entrada de `borders` declara exatamente um alvo: `adjacent_region_id` (`region.*`), `external_boundary_key` ou `natural_obstacle_key`; também possui `boundary_type` (chave controlada) e `notes` opcional. A mesma fronteira não pode ser repetida sob outra descrição.

**Campos opcionais, corpo e proibições.** `aliases`, `summary`, `art` e `notes` seguem o contrato comum. O corpo Markdown usa `Visão geral`, `Geografia`, `Clima`, `População`, `Economia`, `Tradição mágica`, `Contexto político derivado`, `Perigos`, `Reputação`, `História` e `Observações editoriais`. Ele explica dados já estruturados, sem criar métricas concorrentes. São proibidos: listas manuais de assentamentos, locais, rotas ou facções; reino duplicado; estado simulado; valores de recurso, população, bioma, economia ou conflito não estruturados.
...
**Campos obrigatórios.** Além dos comuns: `region_id` (`region.*`), `settlement_type` (chave controlada), `administrative_roles` (lista de chaves controladas), `initial_state` (mapa), `production`, `consumption`, `trade`, `initial_stocks`, `initial_prices`, `local_problems` (listas de mapas) e `references` (lista).
...
`initial_stocks` usa exatamente um `resource_id` ou `item_id`, `quantity`, `quality_key`, `storage_location_id` opcional, `reserved_quantity` opcional e `notes`. `initial_prices` usa exatamente um `resource_id` ou `item_id` e um mapa `price` completo. `reserved_quantity` não pode ser negativa, usa unidade compatível e não supera o estoque inicial conhecido. `local_problems` contém `problem_key`, `severity`, `visibility`, `related_entity_ids` e `notes`; quando problemas se tornarem entidades próprias, a relação deve usar seus IDs.

`initial_state.ruler_id` é a fonte canônica para o governante no início da campanha. Ocupante atual e demais ocupantes serão registrados pelos futuros contratos de NPC e pelo estado da campanha. Governo institucional, presença de facções e instituições são derivados das facções; leis são derivadas por jurisdição; difusão e contexto religiosos são derivados das religiões e leis aplicáveis. O assentamento não mantém listas inversas dessas entidades.
...
**Corpo, dados derivados e validação.** O corpo usa `Visão geral`, `Posição regional`, `Governo derivado`, `População`, `Bairros e organização`, `Economia`, `Comércio`, `Instituições derivadas`, `Contexto religioso derivado`, `Magia`, `Cotidiano`, `Problemas locais`, `Eventos recentes`, `História` e `Observações editoriais`. Bairros, edifícios, instituições, locais, rotas, reino, governo, leis, religiões e população calculada não são listas de pertencimento manuais; são referências canônicas em seus proprietários ou consultas derivadas. A revisão confere região, distinção entre tipo e papel administrativo, faixas, não negatividade, exclusividade entre recurso e item, compatibilidade de unidades, resolução monetária, categorias de ID, ausência de estado atual, ausência de listas inversas, ausência de placeholders e referências aprovadas existentes. Vocabulários de períodos, problema e comércio permanecem adiados.
...
**Corpo, proibições e validação.** O corpo usa `Visão geral`, `Função`, `Localização`, `Acesso`, `Controle institucional derivado`, `Características`, `Recursos`, `Perigos`, `Propriedades mágicas`, `Segredos`, `Acontecimentos relacionados`, `História` e `Observações editoriais`. Controle institucional inicial é derivado de `faction.initial_state.territorial_presence`. São proibidos parentesco espacial ou controle em listas invertidas, ciclos, estado simulado e a revelação de segredo só pela prosa. A revisão confere região, consistência de assentamento, cadeia de pais, acesso, separação das quatro dimensões e referências resolvidas. Vocabulários de descoberta, interface, perigo, propriedade, agenda e requisitos ficam adiados.
...
**Campos obrigatórios.** Além dos comuns: `origin_id`, `destination_id`, `directionality`, `route_type`, `distance`, `base_travel_time`, `access_modes`, `initial_state`, `seasonal_access`, `risk_profile` e `references`.
