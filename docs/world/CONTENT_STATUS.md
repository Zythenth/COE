...
Fase 0 concluída. Fase 1 concluída e encerrada após auditoria editorial final aprovada. A Fase 2 está em andamento: existe 1 mundo planejado e 1 mundo real, `world.eldrath`, `approved`; existe 1 calendário real, `calendar.marco`, também `approved`; e existem 3 idiomas reais, todos `in_review`. O contrato conceitual de geração opcional por projetos públicos do GitHub permanece concluído. Objetivos, medos, segredos, memórias, crenças, conhecimento e relações usam subregistros incorporados; eventos concretos, transmissões, grupos, hordas, ataques, incursões e demais estados atuais pertencem ao save. As quantidades existentes contam entidades reais nos arquivos de conteúdo, não menções, schemas ou templates. Todas as categorias não mencionadas continuam com quantidade existente igual a zero. A próxima atividade é aprovar os idiomas e criar as culturas.
...
- idiomas: `language.valdren`, `language.ardren` e `language.senkar` criados e em revisão; quantidade existente igual a 3;
- Fase 2: em andamento; próxima atividade é a aprovação dos idiomas e a criação das culturas.
...
|---|---:|---:|---|---|---|---|---|---|
| Mundo | 1 | 1 | aprovado | contratos editoriais fundamentais | Fase 2 | `content/worlds/eldrath/world.yaml` | `world.eldrath` preserva a identidade geral e referencia `calendar.marco`, `language.valdren`, `language.ardren` e `language.senkar`; data inicial, era inicial, culturas, reino e geografia permanecem fora do escopo atual. | 2026-07-16 |
...
| Idiomas | a definir | 3 | em revisão | identidade geral e contratos editoriais | Fase 2 | `content/worlds/eldrath/languages/` | `language.valdren` é a língua franca viva; `language.ardren` é a língua clássica documental; `language.senkar` é a língua viva independente. O GDD não fixa quantidade total, e a categoria não está concluída enquanto os três arquivos permanecem em revisão. | 2026-07-16 |
...
O estado `não iniciado` significa que a pasta foi reservada, mas não contém entidades. Existem cinco entidades reais: `world.eldrath` e `calendar.marco`, aprovados, além de `language.valdren`, `language.ardren` e `language.senkar`, em revisão. As demais metas canônicas continuam com suas quantidades existentes registradas na tabela, sem cultura, região, assentamento, facção, religião, família, reino ou NPC criado. Schemas e templates concluídos, inclusive os de criaturas, doenças, NPCs, eventos, rumores e conflitos, não alteram essas quantidades. O contrato GitHub não cria categoria contável, não altera as contagens de Eldrath e não substitui o cenário canônico. Saúde e condições são estruturas compartilhadas; objetivos, medos, segredos, memórias, crenças, conhecimento e relações do NPC são subregistros incorporados. Hordas, ataques e incursões concretos são estado da campanha, não categorias contáveis de conteúdo inicial. Alterações posteriores devem atualizar quantidade existente, estado, documento responsável e data somente depois que o conteúdo real for criado e validado.
