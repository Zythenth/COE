...
Fase 0 concluída. Fase 1 concluída e encerrada após auditoria editorial final aprovada. A Fase 2 está em andamento: existe 1 mundo planejado e 1 mundo real, `world.eldrath`, `approved`; existe 1 calendário real, `calendar.marco`, também `approved`; existem 3 idiomas reais, todos `approved`; e existem 4 culturas fundamentais, todas `in_review`. O contrato conceitual de geração opcional por projetos públicos do GitHub permanece concluído. Objetivos, medos, segredos, memórias, crenças, conhecimento e relações usam subregistros incorporados; eventos concretos, transmissões, grupos, hordas, ataques, incursões e demais estados atuais pertencem ao save. As quantidades existentes contam entidades reais nos arquivos de conteúdo, não menções, schemas ou templates. Todas as categorias não mencionadas continuam com quantidade existente igual a zero. A quantidade total planejada de culturas permanece a definir. A próxima atividade é aprovar as culturas e definir a cosmologia necessária e os princípios religiosos gerais.
...
- culturas: `culture.dorven`, `culture.namaru`, `culture.tessari` e `culture.ostelar` criadas e em revisão; quantidade existente igual a 4 e quantidade planejada a definir;
- Fase 2: em andamento; próxima atividade é a aprovação das culturas e a definição da cosmologia necessária e dos princípios religiosos gerais.
...
|---|---:|---:|---|---|---|---|---|---|
| Mundo | 1 | 1 | aprovado | contratos editoriais fundamentais | Fase 2 | `content/worlds/eldrath/world.yaml` | `world.eldrath` preserva a identidade geral e referencia `calendar.marco`, os três idiomas e as quatro culturas fundamentais; data inicial, era inicial, reino e geografia permanecem fora do escopo atual. | 2026-07-16 |
...
| Culturas | a definir | 4 | em revisão | identidade, calendário e idiomas | Fase 2 | `content/worlds/eldrath/cultures/` | Quatro fundações iniciais: `culture.dorven`, `culture.namaru`, `culture.tessari` e `culture.ostelar`; não representam todas as culturas possíveis, não correspondem automaticamente às quatro regiões futuras e admitem presença multirregional e múltiplas influências por pessoa. | 2026-07-16 |
...
O estado `não iniciado` significa que a pasta foi reservada, mas não contém entidades. Existem nove entidades reais: `world.eldrath`, `calendar.marco`, `language.valdren`, `language.ardren` e `language.senkar`, aprovados, além de `culture.dorven`, `culture.namaru`, `culture.tessari` e `culture.ostelar`, em revisão. As demais metas canônicas continuam com suas quantidades existentes registradas na tabela, sem região, assentamento, facção, religião, família, reino ou NPC criado. Schemas e templates concluídos, inclusive os de criaturas, doenças, NPCs, eventos, rumores e conflitos, não alteram essas quantidades. O contrato GitHub não cria categoria contável, não altera as contagens de Eldrath e não substitui o cenário canônico. Saúde e condições são estruturas compartilhadas; objetivos, medos, segredos, memórias, crenças, conhecimento e relações do NPC são subregistros incorporados. Hordas, ataques e incursões concretos são estado da campanha, não categorias contáveis de conteúdo inicial. Alterações posteriores devem atualizar quantidade existente, estado, documento responsável e data somente depois que o conteúdo real for criado e validado.
