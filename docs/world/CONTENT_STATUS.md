...
Fase 0 concluída. Fase 1 concluída e encerrada após auditoria editorial final aprovada. A Fase 2 está em andamento: existe 1 mundo planejado e 1 mundo real, `world.eldrath`, `in_review` somente pela nova camada cosmológica mínima e pelos princípios religiosos gerais; existe 1 calendário real, `calendar.marco`, `approved`; existem 3 idiomas reais, todos `approved`; e existem 4 culturas fundamentais, todas `approved`. A identidade geral de `world.eldrath` anteriormente aprovada permanece preservada. O contrato conceitual de geração opcional por projetos públicos do GitHub permanece concluído. Objetivos, medos, segredos, memórias, crenças, conhecimento e relações usam subregistros incorporados; eventos concretos, transmissões, grupos, hordas, ataques, incursões e demais estados atuais pertencem ao save. As quantidades existentes contam entidades reais nos arquivos de conteúdo, não menções, schemas ou templates. Todas as categorias não mencionadas continuam com quantidade existente igual a zero; tradições religiosas permanecem com quantidade existente igual a zero e `religions/` contém somente `.gitkeep`. A quantidade total planejada de culturas permanece a definir. A próxima atividade é aprovar a cosmologia e os princípios religiosos gerais, restaurar `world.eldrath` para `approved` e criar o reino, seu sistema político e suas regras de sucessão.
...
- identidade geral de Eldrath: conteúdo anteriormente aprovado preservado; `world.eldrath` em revisão somente pela nova camada cosmológica mínima e pelos princípios religiosos gerais;
...
- cosmologia necessária e princípios religiosos gerais: definidos em `world.eldrath` e em revisão; nenhuma tradição religiosa concreta criada;
- Fase 2: em andamento; próxima atividade é aprovar a nova camada de `world.eldrath`, restaurá-la para `approved` e criar o reino, seu sistema político e suas regras de sucessão.
...
|---|---:|---:|---|---|---|---|---|---|
| Mundo | 1 | 1 | em revisão | contratos editoriais fundamentais | Fase 2 | `content/worlds/eldrath/world.yaml` | A identidade geral aprovada foi preservada. A cosmologia mínima e os princípios religiosos gerais foram registrados em `notes` estruturadas e aguardam aprovação; data inicial, era inicial, reino e geografia permanecem fora do escopo atual. | 2026-07-16 |
...
O estado `não iniciado` significa que a pasta foi reservada, mas não contém entidades. Existem nove entidades reais: `calendar.marco`, `language.valdren`, `language.ardren`, `language.senkar`, `culture.dorven`, `culture.namaru`, `culture.tessari` e `culture.ostelar` estão aprovados; `world.eldrath` está em revisão somente pela nova camada cosmológica e religiosa geral, com sua identidade anterior preservada. As demais metas canônicas continuam com suas quantidades existentes registradas na tabela, sem região, assentamento, facção, religião, família, reino ou NPC criado. Schemas e templates concluídos, inclusive os de criaturas, doenças, NPCs, eventos, rumores e conflitos, não alteram essas quantidades. O contrato GitHub não cria categoria contável, não altera as contagens de Eldrath e não substitui o cenário canônico. Saúde e condições são estruturas compartilhadas; objetivos, medos, segredos, memórias, crenças, conhecimento e relações do NPC são subregistros incorporados. Hordas, ataques e incursões concretos são estado da campanha, não categorias contáveis de conteúdo inicial. Alterações posteriores devem atualizar quantidade existente, estado, documento responsável e data somente depois que o conteúdo real for criado e validado.
